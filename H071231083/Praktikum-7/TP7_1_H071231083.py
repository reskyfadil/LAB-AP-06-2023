...
def awal_trx_history():
    ...
    with open(trx_history, mode="w") as history:
...
def awal_struk(FileStruk, NamaKasir, tampung):
    ...
    with open(FileStruk, mode="a") as struk:
        ...
        for barang in tampung:
            ...
            struk.write(f"|{str(barang['Produk']).ljust(14)}|{('Rp' + str(barang['Harga'])).rjust(14)}|{str(barang['Jumlah']).center(13)}|{('Rp' + str(total_perbarang)).center(14)}|".center(70) + "\n") 
            # struk.write("\n")
        struk.write(f"{('='*60).center(70)}\n")
